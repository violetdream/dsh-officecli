...
export class WorkspaceManager {
  ...
  /** 会话目录（懒创建）：<root>/<sessionId>/，始终为绝对路径。 */
  sessionDir(sessionId: string): string {
    const dir = join(this.root, sanitizeSession(sessionId))
   ...
   */
  resolve(sessionId: string, filename: string): string {
    ...
    return join(this.sessionDir(sessionId), filename)
  }

  /** 列出会话内的 office 文档。 */
  listFiles(sessionId: string): SessionFile[] {
